...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleApplicationPhotoUploadMob(appId) {
        if (typeof window.handleApplicationPhotoUploadProcess === 'function') {
            window.handleApplicationPhotoUploadProcess(appId, { isMobile: true });
        } else {
        ...
        displayApps.forEach(app => {
            ...
            card.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
                    <h5 style="font-size: 1.36rem; font-weight: 700; color: var(--text-primary); margin: 0; line-height: 1.25;">${escapeHtml(app.storeName || app.shopName || '-')}</h5>
                    <div>${statusBadge}</div>
                </div>
        ...
        if (constProgressListMob) {
            ...
            if (filteredConstJobs.length === 0) {
                ...
                filteredConstJobs.forEach(job => {
                    ...
                    // 간판 종류 설정 (플렉스 간판, LED 채널 간판, 돌출 간판, 그외 기타)
   ...
   - 단, 필수 항목 동의를 거부하실 경우 3초 간편 문의 접수 서비스 이용이 제한됩니다.`
    ...
    if (profileEditForm) {
        profileEditForm.addEventListener('submit', (e) => {
            ...
            // users 배열에서 찾아 수정 (DataStore SSOT)
            ...
            if (idx !== -1) {
                ...
                if (window.SupabaseSync && typeof window.SupabaseSync.upsertUser === 'function') {
                    window.SupabaseSync.upsertUser(updatedUser).catch(err => {
                        console.warn('[App] Supabase upsertUser profile err:', err);
                    });
  ...
  async function handlePhotoFiles(fileList) {
    ...
    for (let i = 0; i < filesToProcess.length; i++) {
      ...
      try {
        ...
        console.error('Image processing error in app.js:', err);
      }
    }
