...
// --- 신청서 목록 상태 변경 글로벌 핸들러 (0초 즉시 전역 등록) ---
...
window.openEditApplicationModal = function(appId) {
  ...
  modal.innerHTML = `
    <div style="background: #ffffff; width: 100%; max-width: 540px; max-height: 90vh; overflow-y: auto; border-radius: 16px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.35); border: 1px solid #cbd5e1; padding: 24px; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; position: relative; z-index: 10000000;">
      ...
      <form id="form-edit-application" onsubmit="window.submitEditApplicationModal('${app.id}', event)" style="display: flex; flex-direction: column; gap: 14px;">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
          <div>
            ...
            <input type="text" value="${window.safeHtmlForEditApp(app.id)}" disabled style="width: 100%; padding: 8px 10px; font-size: 0.88rem; font-family: monospace; font-weight: 700; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; color: #64748b; box-sizing: border-box;">
          </div>
          <div>
        ...
        <div>
          ...
          <input type="text" id="edit-app-store-name" value="${window.safeHtmlForEditApp(app.storeName || app.shopName || '')}" required style="width: 100%; padding: 9px 12px; font-size: 0.92rem; border: 1.5px solid #cbd5e1; border-radius: 8px; box-sizing: border-box; font-weight: 600;">
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
          <div>
            ...
            <input type="text" id="edit-app-owner-name" value="${window.safeHtmlForEditApp(app.ownerName || app.name || '')}" required style="width: 100%; padding: 9px 12px; font-size: 0.92rem; border: 1.5px solid #cbd5e1; border-radius: 8px; box-sizing: border-box;">
          </div>
          <div>
            ...
            <input type="text" id="edit-app-owner-phone" value="${window.safeHtmlForEditApp(app.ownerPhone || app.phone || '')}" required style="width: 100%; padding: 9px 12px; font-size: 0.92rem; border: 1.5px solid #cbd5e1; border-radius: 8px; box-sizing: border-box;">
          </div>
        </div>
        ...
        <div>
          ...
          <input type="text" id="edit-app-store-address" value="${window.safeHtmlForEditApp(app.storeAddress || app.address || '')}" required style="width: 100%; padding: 9px 12px; font-size: 0.92rem; border: 1.5px solid #cbd5e1; border-radius: 8px; box-sizing: border-box;">
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
          <div>
            <label style="display: block; font-size: 0.82rem; font-weight: 700; color: #1e293b; margin-bottom: 4px;">희망 간판 종류</label>
            <input type="text" id="edit-app-sign-type" value="${window.safeHtmlForEditApp(app.signType || '')}" placeholder="예: LED 채널 간판" style="width: 100%; padding: 9px 12px; font-size: 0.92rem; border: 1.5px solid #cbd5e1; border-radius: 8px; box-sizing: border-box;">
          </div>
          <div>
            <label style="display: block; font-size: 0.82rem; font-weight: 700; color: #1e293b; margin-bottom: 4px;">담당 영업자 코드</label>
            <input type="text" id="edit-app-referrer-code" value="${window.safeHtmlForEditApp(app.referrerCode || '')}" placeholder="예: B-260903" style="width: 100%; padding: 9px 12px; font-size: 0.92rem; border: 1.5px solid #cbd5e1; border-radius: 8px; box-sizing: border-box; font-family: monospace;">
          </div>
        </div>

        <div>
          <label style="display: block; font-size: 0.82rem; font-weight: 700; color: #1e293b; margin-bottom: 4px;">관리자 메모 / 비고</label>
          <textarea id="edit-app-memo" rows="2" placeholder="관리자 메모 입력" style="width: 100%; padding: 9px 12px; font-size: 0.9rem; border: 1.5px solid #cbd5e1; border-radius: 8px; box-sizing: border-box; resize: vertical;">${window.safeHtmlForEditApp(app.memo || app.notes || '')}</textarea>
        </div>
...
// --- 신청 상태 뱃지 헬퍼 (영업자/관리자/시공사는 깔끔한 단일 뱃지 원상복구, 일반회원에게만 공단 진행상황 듀얼 뱃지 적용) ---
window.getAppStatusBadgeHtml = function(app) {
  if (!app) return '';
  ...
  const safe = typeof window.safeHtmlForEditApp === 'function' ? window.safeHtmlForEditApp : (t => String(t || ''));

  // 1단계: 사전 등록 심사 상태 뱃지 (공통)
  let preBadgeHtml = '';
  if (s === 'approved' || s === '서류준비 & 접수대기' || s === '서류제출 & 접수예정' || s === '서류제출&접수예정' || s === '승인 완료' || s === '승인완료') {
    preBadgeHtml = `<span style="background: #dcfce7; color: #166534; border: 1.5px solid #86efac; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-file-signature"></i> 서류준비 & 접수대기</span>`;
  } else if (s === 'unqualified' || s === '신청요건 미달업체' || s === '신청요건미달' || s === '미달') {
    preBadgeHtml = `<span style="background: #ffedd5; color: #c2410c; border: 1.5px solid #fed7aa; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-triangle-exclamation"></i> 신청요건 미달업체</span>`;
  } else if (s === 'rejected' || s === '지원사업 탈락' || s === '지원사업탈락' || s === '반려됨') {
    preBadgeHtml = `<span style="background: #fee2e2; color: #991b1b; border: 1.5px solid #fca5a5; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-circle-xmark"></i> 지원사업 탈락</span>`;
  } else if (s === 'giveup' || s === '지원사업 포기' || s === '지원사업포기') {
    preBadgeHtml = `<span style="background: #fffbeb; color: #b45309; border: 1.5px solid #fde68a; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-ban"></i> 지원사업 포기</span>`;
  } else {
    preBadgeHtml = `<span style="background: #f1f5f9; color: #475569; border: 1.5px solid #cbd5e1; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-regular fa-clock"></i> 사업시행 전 사전등록업체</span>`;
  }

  // 현재 사용자 역할 확인 (영업자, 최고관리자, 시공사는 영업물건 전용 테이블이 따로 있으므로 신청서 목록은 기존대로 깔끔한 단일 뱃지 유지)
  let currentRole = 'user';
  try {
    const au = JSON.parse(localStorage.getItem('activeUser') || sessionStorage.getItem('activeUser') || '{}');
  ...
  if (rStatus === '접수완료' || rStatus.includes('접수완료') || rStatus === '접수 완료') {
    receiptBadge = `<span style="background: #e0f2fe; color: #0369a1; border: 1.5px solid #7dd3fc; padding: 3px 8px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-building-columns"></i> 공단 접수완료</span>`;
  } else if (rStatus === '접수예정' || rStatus.includes('접수예정')) {
    receiptBadge = `<span style="background: #fef3c7; color: #92400e; border: 1.5px solid #fde68a; padding: 3px 8px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-clock"></i> 공단 접수예정</span>`;
  } else if (isBizItem || rStatus === '업체신청') {
    receiptBadge = `<span style="background: #f1f5f9; color: #334155; border: 1.5px solid #cbd5e1; padding: 3px 8px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-file-lines"></i> 업체신청</span>`;
  } else {
    receiptBadge = `<span style="background: #f8fafc; color: #64748b; border: 1px solid #e2e8f0; padding: 2px 7px; border-radius: 4px; font-size: 0.72rem; font-weight: 600; display: inline-flex; align-items: center; gap: 3px;"><i class="fa-regular fa-clock"></i> 공단 공고 대기</span>`;
  }

  // 일반회원: 공단 진행상황 5단계 뱃지 생성
  let progressBadge = '';
  if (pStatus === '간판시공완료' || pStatus === 'completed' || pStatus === '시공완료' || pStatus === '정산 완료') {
    progressBadge = `<span style="background: #ede9fe; color: #6d28d9; border: 1.5px solid #c4b5fd; padding: 3px 8px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-award"></i> 간판시공완료</span>`;
  } else if (pStatus === '간판시공 준비중' || pStatus === 'in_construction' || pStatus === '시공준비') {
    progressBadge = `<span style="background: #fef9c3; color: #a16207; border: 1.5px solid #fef08a; padding: 3px 8px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-screwdriver-wrench"></i> 간판시공 준비중</span>`;
  } else if (pStatus === '대상자선정' || pStatus === '선정' || pStatus === 'before_construction') {
    progressBadge = `<span style="background: #dcfce7; color: #15803d; border: 1.5px solid #86efac; padding: 3px 8px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-circle-check"></i> 대상자선정</span>`;
  } else if (pStatus === '심사대기중' || pStatus === '심사대기') {
    progressBadge = `<span style="background: #fef3c7; color: #b45309; border: 1.5px solid #fde68a; padding: 3px 8px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-magnifying-glass"></i> 심사대기중</span>`;
  } else if (isBizItem || (pStatus && pStatus !== 'none')) {
    progressBadge = `<span style="background: #f1f5f9; color: #475569; border: 1.5px solid #cbd5e1; padding: 3px 8px; border-radius: 6px; font-size: 0.74rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-hourglass-start"></i> 지원대기중</span>`;
  } else {
    progressBadge = `<span style="background: #f8fafc; color: #94a3b8; border: 1px solid #e2e8f0; padding: 2px 7px; border-radius: 4px; font-size: 0.72rem; font-weight: 500; display: inline-flex; align-items: center; gap: 3px;"><i class="fa-solid fa-info-circle"></i> 접수 대기중</span>`;
  }

  // 일반회원: 배정 시공사 뱃지
  let constBadge = '';
  if (constName && constName !== 'none' && constName !== '미배정') {
    constBadge = `<div style="font-size: 0.73rem; color: #1d4ed8; font-weight: 700; background: #eff6ff; border: 1.5px solid #bfdbfe; border-radius: 6px; padding: 3px 8px; display: inline-flex; align-items: center; gap: 5px; margin-top: 4px;"><i class="fa-solid fa-hard-hat" style="color: #2563eb;"></i> 배정 시공업체: <strong>${safe(constName)}</strong></div>`;
  }

  return `
    <div style="display: flex; flex-direction: column; gap: 6px; align-items: flex-start; min-width: 190px;">
...
window.clearAllInquiriesAdmin = function() {
  ...
  (typeof DataStore !== 'undefined' && DataStore.saveInquiries ? DataStore.saveInquiries([]) : localStorage.setItem('inquiries', JSON.stringify([])));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (activeUser && activeUser.role === 'admin') {
      ...
      fetch(_sbUrl + '/rest/v1/inquiries?select=*&order=created_at.desc', {
        ...
        if (Array.isArray(rows)) {
          ...
          (typeof DataStore !== 'undefined' && DataStore.saveInquiries ? DataStore.saveInquiries(mapped) : localStorage.setItem('inquiries', JSON.stringify(mapped)));
          if (typeof renderInquiriesList === 'function') renderInquiriesList();
        }
      }).catch(() => {});
  ...
  window.addEventListener('storage', (e) => {
    if (e.key === 'applications' || e.key === 'users' || e.key === 'inquiries' || e.key === 'ganpan_cross_tab_sync') {
      ...
      applications = JSON.parse(localStorage.getItem('applications')) || [];
      if (activeUser) {
        const freshActive = users.find(u => u.id === activeUser.id);
        if (freshActive) {
          activeUser = sanitizeUser(freshActive);
        }
      }
      if (activeUser && activeUser.role === 'admin') {
        renderAllUsersList();
  ...
  if (constructorRequestForm && constructorModal) {
    constructorRequestForm.addEventListener('submit', (e) => {
      ...
      if (window.DataStore && typeof window.DataStore.lockUserUpdate === 'function') {
        window.DataStore.lockUserUpdate(activeUser.id, updatedFields);
      }
      ...
      constructorModal.style.display = 'none';
      constBusinessNameInput.value = '';
      constLicenseNumberInput.value = '';
  ...
  if (mobileUploadForm) {
    mobileUploadForm.addEventListener('submit', async (e) => {
      ...
      // Supabase 클라우드 DB 실시간 단일 원천 동기화
      if (window.SupabaseSync) {
        try {
  ...
  const renderApplicationsList = () => {
    ...
    paginatedApps.forEach(app => {
      ...
      // 영업담당자 이름 매칭 (예: 김만석영업자 또는 본사직접접수)
      let bizUserName = '';
      if (app.salespersonName === '본사직접접수' || app.salespersonName === '본사 직접 접수' || (!app.referrerCode && !app.salespersonId && !app.salespersonName)) {
        ...
        bizUserName = app.salespersonName.endsWith('영업자') ? app.salespersonName : `${app.salespersonName}영업자`;
    ...
    paginatedJobs.forEach(job => {
      ...
      if (draftCount > 0) {
        ...
        if (job.draftStatus === 'owner_approved') {
          draftStatusHtml = `<span style="background: #dcfce7; color: #166534; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.86rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-circle-check"></i> 점주 시안확정</span>`;
        } else if (job.draftStatus === 'admin_approved') {
          draftStatusHtml = `
            <div style="display: flex; flex-direction: column; align-items: center; gap: 4px;">
              <span style="background: #dbeafe; color: #1e40af; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.86rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-user-shield"></i> 관리자 직권확정</span>
              <button type="button" onclick="window.toggleDraftApproval('${job.id}', 'pending')" style="background: none; border: none; color: #94a3b8; text-decoration: underline; font-size: 0.82rem; cursor: pointer;">확정 취소</button>
            </div>
          `;
        } else {
          draftStatusHtml = `
            <div style="display: flex; flex-direction: column; align-items: center; gap: 4px;">
              <span style="background: #fef3c7; color: #92400e; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.86rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-clock"></i> 시안 검토중</span>
              <button type="button" onclick="window.toggleDraftApproval('${job.id}', 'admin_approved')" style="padding: 4px 10px; font-size: 0.86rem; font-weight: 700; background: #2563eb; color: white; border: none; border-radius: 5px; cursor: pointer; display: inline-flex; align-items: center; gap: 4px;">
        ...
        draftBadge = `
          <div style="display: flex; flex-direction: column; align-items: center; gap: 6px;">
            <div style="display: flex; gap: 5px; align-items: center;">
              <button type="button" onclick="window.viewDraftModal('${job.id}')" style="background: #f5f3ff; color: #7c3aed; border: 1px solid #ddd6fe; padding: 5px 10px; border-radius: 6px; font-size: 0.88rem; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 4px;" title="디자인 시안 보기 및 삭제">
                <i class="fa-solid fa-palette"></i> 시안 (${draftCount}/5장)
              </button>
              ${draftCount < 5 ? `
                <label style="background: #ede9fe; color: #6d28d9; border: 1px solid #c4b5fd; padding: 5px 9px; border-radius: 6px; font-size: 0.86rem; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 3px;" title="시안 추가 업로드 (최대 5장, 300KB 자동 압축)">
        ...
        draftBadge = `
          <div style="display: flex; flex-direction: column; align-items: center; gap: 5px;">
            <span style="color: #94a3b8; font-size: 0.88rem;"><i class="fa-solid fa-hourglass-start"></i> 시안 미등록</span>
            <label style="background: #ede9fe; color: #6d28d9; border: 1px solid #c4b5fd; padding: 5px 10px; border-radius: 6px; font-size: 0.86rem; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 3px;" title="시안 신규 등록 (최대 5장, 300KB 자동 압축)">
      ...
      const uploadConstBtn = (afterCount < 5) ? `
        <label style="background: #10b981; color: #ffffff; border: 1px solid #059669; padding: 4px 8px; border-radius: 6px; font-size: 0.75rem; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 4px;" title="시공 후 사진 등록 (최대 5장, 300KB 자동 압축)">
      ...
      if (afterCount > 0) {
        ...
        viewConstBtn = '<span style="color: #94a3b8; font-size: 0.75rem;">(미등록)</span>';
      }
      ...
      tr.innerHTML = `
        ...
        <td style="padding: 14px 16px; text-align: center; white-space: nowrap;">
          <select class="status-select select-admin-const-status" data-id="${job.id}" style="padding: 6px 10px; font-size: 0.90rem; font-weight: 700; border-radius: 6px; border: 1.5px solid ${statusBorder}; color: ${statusColor}; background: ${statusBg}; cursor: pointer;">
  ...
  // PC 신청서 현장사진 파일 선택/업로드 핸들러 (SSOT 마스터 함수로 일원화)
  const handleApplicationPhotoUploadPC = (appId) => {
    if (typeof window.handleApplicationPhotoUploadProcess === 'function') {
      window.handleApplicationPhotoUploadProcess(appId, { isMobile: false });
    } else {
      ...
      <div style="background: #ffffff; width: 100%; max-width: 540px; max-height: 90vh; overflow-y: auto; border-radius: 16px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.35); border: 1px solid #cbd5e1; padding: 24px; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; position: relative; z-index: 10000000;">
        ...
        <form id="form-edit-application" onsubmit="window.submitEditApplicationModal('${app.id}', event)" style="display: flex; flex-direction: column; gap: 14px;">
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
            <div>
              <label style="display: block; font-size: 0.8rem; font-weight: 700; color: #475569; margin-bottom: 4px;">신청번호 (고유코드)</label>
              <input type="text" value="${safeHtml(app.id)}" disabled style="width: 100%; padding: 8px 10px; font-size: 0.88rem; font-family: monospace; font-weight: 700; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; color: #64748b; box-sizing: border-box;">
            </div>
            <div>
              <label style="display: block; font-size: 0.8rem; font-weight: 700; color: #475569; margin-bottom: 4px;">진행 상태</label>
              <select id="edit-app-status" style="width: 100%; padding: 8px 10px; font-size: 0.88rem; font-weight: 700; border: 1.5px solid #cbd5e1; border-radius: 6px; box-sizing: border-box; background: #ffffff; cursor: pointer;">
  ...
  // 이벤트 위임으로 .btn-edit-app 클릭 시에도 확실하게 모달 열기 보장
  ...
  // 신청 상태 뱃지 헬퍼 (최상단 window.getAppStatusBadgeHtml 단일 원천 위임)
  const getAppStatusBadgeHtml = (app) => {
    if (typeof window.getAppStatusBadgeHtml === 'function') {
      return window.getAppStatusBadgeHtml(app);
    }
    const s = String((app && app.status) || '').trim();
    if (s === 'approved' || s === '서류준비 & 접수대기' || s === '서류제출 & 접수예정' || s === '서류제출&접수예정' || s === '승인 완료' || s === '승인완료') {
      return `<span style="background: #dcfce7; color: #166534; border: 1.5px solid #86efac; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-file-signature"></i> 서류준비 & 접수대기</span>`;
    } else if (s === 'unqualified' || s === '신청요건 미달업체' || s === '신청요건미달' || s === '미달') {
      return `<span style="background: #ffedd5; color: #c2410c; border: 1.5px solid #fed7aa; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-triangle-exclamation"></i> 신청요건 미달업체</span>`;
    } else if (s === 'rejected' || s === '지원사업 탈락' || s === '지원사업탈락' || s === '반려됨') {
      return `<span style="background: #fee2e2; color: #991b1b; border: 1.5px solid #fca5a5; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-circle-xmark"></i> 지원사업 탈락</span>`;
    } else if (s === 'giveup' || s === '지원사업 포기' || s === '지원사업포기') {
      return `<span style="background: #fffbeb; color: #b45309; border: 1.5px solid #fde68a; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-solid fa-ban"></i> 지원사업 포기</span>`;
    } else {
      return `<span style="background: #f1f5f9; color: #475569; border: 1.5px solid #cbd5e1; padding: 4px 10px; border-radius: 9999px; font-weight: 700; font-size: 0.76rem; display: inline-flex; align-items: center; gap: 4px;"><i class="fa-regular fa-clock"></i> 사업시행 전 사전등록업체</span>`;
    }
  };
  ...
  const renderUserApplicationsList = () => {
    ...
    paginatedApps.forEach(app => {
      ...
      tr.innerHTML = `
        ...
        <td style="padding: 12px 16px;">
          <div style="font-weight: 700; color: var(--text-primary); font-size: 0.92rem; line-height: 1.3;">${escapeHtml(app.storeName || '-')}</div>
      ...
      if (job.constructionStatus === 'completed') {
        ...
        actionsHtml = `
          <div style="display: flex; flex-direction: column; gap: 8px; align-items: stretch; padding: 8px;">
            ...
            <div style="background: #fdf4ff; border: 1px solid #f5d0fe; padding: 8px 12px; border-radius: 6px; text-align: left;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                <label style="font-size: 0.88rem; font-weight: 700; color: #86198f;">
                  <i class="fa-solid fa-palette"></i> 간판 디자인 시안 (${draftCount}/5장)
                </label>
                ${draftCount >= 5 ? '<span style="font-size: 0.82rem; color: #dc2626; font-weight: 700;">최대 등록됨</span>' : ''}
              </div>
              <input type="file" class="const-draft-input" data-id="${job.id}" accept="image/*" multiple style="font-size: 0.86rem; width: 100%;">
              ${draftCount > 0 ? `
                <div style="margin-top: 5px; display: flex; align-items: center; justify-content: space-between;">
                  <button type="button" onclick="window.viewDraftModal('${job.id}')" style="padding: 5px 10px; font-size: 0.86rem; background: #ede9fe; color: #6d28d9; border: 1px solid #c4b5fd; border-radius: 5px; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; font-weight: 700;">
            ...
            <div style="background: #f0fdf4; border: 1px solid #bbf7d0; padding: 8px 12px; border-radius: 6px; text-align: left;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                <label style="font-size: 0.88rem; font-weight: 700; color: #166534;">
                  <i class="fa-solid fa-camera"></i> 시공 후 사진 증빙 (${job.constructionPhotos ? job.constructionPhotos.length : 0}/5)
                </label>
              </div>
              <input type="file" class="const-photo-input" data-id="${job.id}" accept="image/*" multiple style="font-size: 0.86rem; width: 100%;">
              ${(job.constructionPhotos && job.constructionPhotos.length > 0) ? `
                <div style="margin-top: 5px; display: flex; align-items: center; justify-content: space-between;">
                  <button type="button" onclick="window.viewConstructionPhotosModal('${job.id}')" style="padding: 5px 10px; font-size: 0.86rem; background: #dcfce7; color: #15803d; border: 1px solid #86efac; border-radius: 5px; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; font-weight: 700;">
            ...
            <div style="display: flex; gap: 8px; justify-content: flex-end; align-items: center; margin-top: 6px;">
              <select class="status-select select-const-status" data-id="${job.id}" style="padding: 6px 10px; font-size: 0.90rem; font-weight: 700; border-radius: 6px; border: 1px solid #cbd5e1; background: white;">
              ...
              <button class="btn btn-primary btn-sm btn-report-job-complete" data-id="${job.id}" style="padding: 7px 14px; font-size: 0.90rem; background: #16a34a; border: none; border-radius: 6px; cursor: pointer; color: white; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;">
                <i class="fa-solid fa-paper-plane"></i> 최종 시공 완료 보고
              </button>
            </div>
          </div>
        `;
